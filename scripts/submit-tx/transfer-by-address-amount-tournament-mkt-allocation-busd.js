...
const BUSD_TOKEN = process.env.BUSD_TOKEN_MAINNET
...
async function main() {
  ...
  const busdToken = new ethers.Contract(BUSD_TOKEN, GFALTokenArtifact.abi, provider);
  ...
  for (let i = 0; i < TRANSFERS_PAYLOAD.address.length; i++) {
    ...
    const gasLimit = await busdToken.estimateGas.transfer(TRANSFERS_PAYLOAD.address[i], TRANSFERS_PAYLOAD.amount[i], {from: signer.address});

    // Construct the transaction
    const tx = {
      from: signer.address, // specify the sender
      to: busdToken.address,
      gasLimit,
      gasPrice,
      nonce: nonce + i,
      data: busdToken.interface.encodeFunctionData('transfer', [TRANSFERS_PAYLOAD.address[i], TRANSFERS_PAYLOAD.amount[i]]),
    };
