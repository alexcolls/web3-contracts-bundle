...
async function main() {
  ...
  const provider = new ethers.providers.JsonRpcProvider(process.env.WEB3_HTTP_PROVIDER_MAIN);
  ...
  for (let i = 0; i < VESTING_CONTRACTS.length; i++) {
    try {
      ...
      let vestingBasic = new ethers.Contract(VESTING_CONTRACTS[i], GFALTokenArtifact.abi, provider);
